...
import { getBookie, getBookies, updateBookie, deleteBookie } from '../services/bookies'

export const useGetBookies = () => {
  const { data, isLoading, isError } = useQuery({ queryKey: ['bookies'], queryFn: getBookies })
  return { data, isLoading, isError }
}
export const useGetBookie = (id) => {
  const { data, isLoading, isError } = useQuery({ queryKey: ['bookie', id], queryFn: () => getBookie(id) })
  return { data, isLoading, isError }
}
