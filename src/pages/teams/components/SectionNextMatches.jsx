...
const SectionNextMatches = ({ team }) => {
  ...
  return (
        ...
        <FormControl style={{ fontSize: '11px' }} placeholder='Busca por equipo, temporada, jornada..'name='filter' value={dataFilter} onChange={(event) => setDataFilter(event.target.value)} />
        </div>
        {(filter?.length > 0)
          ...
          <Table style={{ fontSize: '11px' }} responsive variant='dark table-sm table-borderless' hover>
            ...
            <td><Link style={{ fontSize: '11px' }} className="btn btn-warning btn-sm" to={`/matches/${match?._id}`}>Detalles</Link></td>
            </tr>
            ))}
              </tbody>
            </Table>
